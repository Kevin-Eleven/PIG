...
class PIGConfig:
    ...
    class Model:
        ...
        attn_resolutions = [
            (16, 16),
        ]  # 当图像特征大小在attn_resolutions里时加入注意力模块，长宽比需要与初始分辨率一致
        dropout = 0.0
        resamp_with_conv = (
            True  # 为True时使用卷积进行上/下采样，否则直接插值/池化进行上/下采样
        )
    ...
    class Diffusion:
        ...
        beta_schedule = (
            "linear"  # beta的取值方法:'quad','linear','const','jsd','sigmoid'
        )
        beta_start = 1e-4
        beta_end = 2e-2
    ...
    class Train:
        ...
        x_model_path = os.path.join(
            ".",
            "log",
            "20250106_111708_x_mask_y_img_x",
            "best_epoch120_loss0.2651_x_mask_y_img_x.pth",
        )  # 条件模型路径，训练目标模型时用于生成条件
        binary = False  # 是否进行二值化
        binary_thre_weights = [
            3 / 4,
            1 / 4,
        ]  # 二值化阈值=binary_thre_weights[0]*max+binary_thre_weights[1]*min
        ternary = False  # 是否进行三值化
        ternary_thre_weights = [
            (1 / 3, 2 / 3),
            (7 / 8, 1 / 8),
        ]  # [thre0,thre1],thre_weights[1][0]>thre_weights[0][0]
        loss_weight_flag = True  # 训练目标模型时是否按cond加权
        resume_train = False  # 继续训练
        finetune = False  # 为True时resume_train只加载ckpt_path的模型权重（优化器/学习率/epoch重新开始），用于预训练后微调
        ckpt_path = os.path.join(
            ".",
            "log",
            "20241210_113429_cond_mask_target_img_exp1",
            "ckpt_epoch7_loss13.1026_cond_mask_target_img_exp1.pth",
        )  # 继续训练使用的checkpoint路径
        log_dir = os.path.join(
            ".", "log", f"{time.strftime('%Y%m%d_%H%M%S', time.localtime())}_{desc}"
        )  # checkpoint保存目录
        batch_size = 6
        num_workers = 8
        n_epochs = 128
    ...
    class Sample:
        ...
        y_model_path = os.path.join(
            ".",
            "log",
            "20250206_114514_x_mask_y_img_y_pretrained",
            "best_epoch126_loss3.5979_x_mask_y_img_y_pretrained.pth",
        )  # 采样使用的模型路径
        data_list = ["img", "mask", "masked", "multi_mask", "patient_info"]
        ...
        num_sample_timesteps = (
            1024  # 采样的步数，在扩散的总步数//采样步数!=0时实际采样步数会大于设定值
        )
        ...
        result_dir = os.path.join(
            ".", "result", f"{time.strftime('%Y%m%d_%H%M%S', time.localtime())}_{desc}"
        )
